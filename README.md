...
| `tools/mobile/measure-rate.js` | 实测手机 App 生成 4 的真实概率（每步白拿一个样本） |
...
App 有「撤回」按钮（实测只回退一步、可无限次重复、棋盘和分数一起回退），
但**本脚本按需求不使用它**：每步都用 AI 选一个方向直接落子，走成死局就记录成绩、
点「重置」开新局。想自己验证撤回行为可以看 git 历史里的 `tools/mobile/undo-probe.js`。
...
- 死局后自动点「重置」开新局（并确认新局真的画出来了，不会把过渡帧误判成"0 步死局"），
  每局结果追加到 `mobile-results.jsonl`
- 首次用会有 1~2 步偏慢（设备侧预览/编码预热），之后稳定
- 棋盘矩形固定为 `x 105..1154, y 1085..2134`；换设备或换分辨率需重新校准
- **右侧那个按钮在游戏中是「菜单」，只有出现 GAMEOVER 时才变成「重置」**。
  所以脚本只在"棋盘确实无路可走"时才点它；万一误弹了设置面板，按钮会被挡住、
  画面也被误读，脚本用「撤回」按钮的底色（游戏界面 `#82DADD` / 被面板盖住 `#3ECBD1`）
  判断出界面被遮挡，自动点「返回」关掉后重新识别
