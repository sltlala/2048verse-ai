...
//   node tools/mobile/bot-mobile.js --p4 10         # 生成 4 的概率(%)
...
//
// 策略说明:
//   不使用 App 的"撤回"功能 (按需求关闭)。死局就记录成绩并点"重置"开新局。
//
// 加速要点:
...
//   - 右侧那个按钮游戏中是"菜单", 只有 GAMEOVER 才是"重置", 所以只在真死局时才点它
const fs = require('fs');
...
const SPAWN_STATS = path.join(ROOT, 'mobile-spawn-stats.json');
...
ai.setFourRate(P4);   // 实测手机 App 约 10.45% (1125 样本, 与网站一致), 用 --p4 可改
...
let spawnTwos = 0, spawnFours = 0;   // 本次跑下来的"新方块是 2 还是 4"统计
...
// 只用棋盘区域做识别 (board.js 的快速解码), 顺带拿到"界面有没有被面板盖住"
...
// 校验通过时顺手读出刚生成的新方块是 2 还是 4, 用来复核 --p4 设得对不对
...
async function startNewGame() {
  ...
  timing.restart += Date.now() - t;
}

// 关掉可能误弹出的设置面板 (点面板里的"返回"), 避免按钮被挡住后瞎点
...
(async () => {
  ...
  console.log(`  AI 预算 ${BUDGET}ms/步  最多 ${MAX_MOVES} 步  死局后${RESTART ? '自动开新局' : '退出'}` +
    `${DISCOVER_ONLY ? '  [仅识别模式]' : ''}`);
  console.log(`  生成 4 的概率 ${P4}%  (不使用撤回; 实测工具: node tools/mobile/measure-rate.js 400)`);
  console.log('');
  ...
  while (moves < MAX_MOVES) {
    ...
    // 界面被面板盖住 (例如误点"菜单"弹出了设置面板), 或者读到整盘空白:
    // 这时按钮点不到、棋盘也不可信, 必须先关掉面板重新识别, 不能当成"死局"。
    // 判断依据: "撤回" 按钮底色在游戏界面里是 #82DADD, 被面板盖住时是 #3ECBD1。
    ...
    for (;;) {
      ...
      if (reason === null || tries >= MAX_RETRY) {
        ...
        if (reason === null) {
          const v = spawnOf(r2.board, expected);
          if (v === 2) spawnTwos++; else if (v === 4) spawnFours++;
        }
        pending = { png: p2, board: r2.board, colors: r2.colors, unknown: r2.unknown, covered: r2.covered };
        break;
      }
      tries++;
      const tw = Date.now();
  ...
  if (sv >= 20) {
    ...
    try {
      ...
      try { all = { ...all, ...JSON.parse(fs.readFileSync(SPAWN_STATS, 'utf8')) }; } catch (_) {}
      all.twos += spawnTwos; all.fours += spawnFours; all.runs += 1; all.updatedAt = new Date().toISOString();
      fs.writeFileSync(SPAWN_STATS, JSON.stringify(all, null, 2));
